...
- **h2 exec** = agent writes JS against `h2.run()`, `h2.delegate()`, `h2.wait()`, etc. One Bash call, scriptable orchestration. Supports `{ executor: "gemini" }` for per-task routing.
...
3. Add a `[<name>]` section type to `config.ts::TomlShape`
